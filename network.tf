...
module "vpc" {
  ...
  cidrsubnet(var.network.base_cidr, var.network.public_subnets_newbits, index)]

  private_subnets = [for index in range(var.network.private_subnets_count) :
  cidrsubnet(var.network.base_cidr, var.network.private_subnets_newbits, sum([var.network.public_subnets_count, index]))]
